...
function preloadedRoutes(store, req){
    const loadPage = (listId, service) => {
        ...
        const {page = 1, pageSize = store.getState()[listId].pageSize} = req.query;
        store.dispatch(setPage(page, pageSize));
        
        return service.list({page, pageSize}, req.currentUser) 
            .then(page => {
    ...
    return[{
        path: '/customers',
        load: () => loadPage('cusomers', customersService)
    }, {
        path: '/invoices',
